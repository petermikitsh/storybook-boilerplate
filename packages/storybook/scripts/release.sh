...
npx --no-install gh-pages --dist $SRC_DIR --dest ./$CURR_VERSION --add
npx --no-install gh-pages --dist $SRC_DIR --dest ./latest

# Copy storybook config to gh-pages root
STORYBOOK_DIR=$SUBPACKAGE_DIR/.storybook
npx --no-install gh-pages --dist $STORYBOOK_DIR --src storybook-config.json --add
